...
def prepare_data(zip_root, extract_root):
    os.makedirs(extract_root, exist_ok=True)
    zip_files = sorted(glob.glob(os.path.join(zip_root, "*.zip")))
    if len(zip_files) == 0:
        return {}
    video_path_map = {}
    for zip_path in tqdm(zip_files, desc="Sync"):
...
class FutureFramePredictionDataset(Dataset):
    def __init__(self, video_map, config_dict, augment=False, split_mode='all', split_ratio=0.8):
        self.pairs = []
        self.image_cache = {}
        self.augment = augment
        self.jitter_params = {'brightness': 0.4, 'contrast': 0.4, 'saturation': 0.5, 'hue': 0.1}
        files_to_load = []
        video_files_dict = {}
        ...
        for video_name, files in video_files_dict.items():
            ...
            gaps, min_th = settings['gaps'], settings['min_th']
            valid_files = [f for f in files if f in self.image_cache]
            for gap in gaps:
                for i in range(len(valid_files) - gap):
                    p1, p2 = valid_files[i], valid_files[i+gap]
                    im1, im2 = self.image_cache[p1], self.image_cache[p2]
                    mse = np.mean((im2.astype(np.float32) - im1.astype(np.float32)) ** 2)
                    if mse < min_th or mse > config.GLOBAL_MAX_THRESH: continue
                    self.pairs.append((p1, p2))

    def apply_sync_augmentations(self, img1, img2):
        pil1, pil2 = Image.fromarray(img1), Image.fromarray(img2)
        b = random.uniform(1 - self.jitter_params['brightness'], 1 + self.jitter_params['brightness'])
        c = random.uniform(1 - self.jitter_params['contrast'], 1 + self.jitter_params['contrast'])
        s = random.uniform(1 - self.jitter_params['saturation'], 1 + self.jitter_params['saturation'])
        h = random.uniform(-self.jitter_params['hue'], self.jitter_params['hue'])
        do_flip, do_gray = random.random() > 0.5, random.random() < 0.1
        pil1 = TF.adjust_hue(TF.adjust_saturation(TF.adjust_contrast(TF.adjust_brightness(pil1, b), c), s), h)
        pil2 = TF.adjust_hue(TF.adjust_saturation(TF.adjust_contrast(TF.adjust_brightness(pil2, b), c), s), h)
        if do_gray: pil1, pil2 = TF.to_grayscale(pil1, 3), TF.to_grayscale(pil2, 3)
        if do_flip: pil1, pil2 = TF.hflip(pil1), TF.hflip(pil2)
    ...
    def __getitem__(self, idx):
        ...
        im1, im2 = self.image_cache[p1], self.image_cache[p2]
        if self.augment: im1, im2 = self.apply_sync_augmentations(im1, im2)
